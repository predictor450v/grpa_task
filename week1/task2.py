'''Solve all the below tasks related to relational and logical operators.
This exercise gives you practice in building up boolean expressions.'''

# Problem Type: Input variable - Output Variable, Hidden suffix for evaluation


# Sample inputs (# note: The values given in the prefix code(grey) will be changed by the autograder according to the testcase while running them.
a = int(input('enter value of a: \n'))


# Assume discount is given in percentages

# <eoi>

output1 = a>=5 # bool: True if a greater than or equal to 5
print(f' a greater than or equal to 5 :- {output1}')
output2 = a/5 == True # bool: True if a is divisible by 5
print(f' a is divisible by 5:- {output2}')
output3 = (a%2!=0) and (a<10) # bool: True if a is odd number less than 10
print(f' a is odd number less than 10:- {output3}')
output4 = (a%2!=0) and (-10<a<10) # bool: True if a is an odd number within the range -10 and 10
print(f' a is an odd number within the range -10 and 10:- {output4}')
output5 =len(str(a))%2==0 and (len(str(a))<10) # bool: True if a has even number of digits but not more than 10 digits
print(f' a has even number of digits but not more than 10 digits:- :- {output5}')

price1, discount1 = (int(input('price1\n')),int(input('discount1%')))
price2, discount2 = (int(input('price2\n')),int(input('discount2%')))
discounted_price1 = float(price1-(price1*(discount1/100)))
discounted_price2 =float(price2-(price2*(discount2/100)))

is_offer1_cheaper = discounted_price1<discounted_price2 # bool: True if the offer1 is strictly cheaper
print(f'offer 1 is cheaper:-{is_offer1_cheaper}')

